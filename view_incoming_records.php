...
if(!empty($search)){
    ...
    foreach($keywords as $word){
        ...
        if($word !== ''){
            $conditions[] = "(
                LOWER(SENDER) LIKE '%$word%' OR
                LOWER(SUBJECT) LIKE '%$word%' OR
                LOWER(FN) LIKE '%$word%' OR
                LOWER(ACTION_TAKEN) LIKE '%$word%' OR
...
<head>
<meta charset="UTF-8">
<title>Incoming Records | Legislative System</title>
...
/* ===== SPREADSHEET HOVER PREVIEW (ONLY FUNCTION ADDED) ===== */
...
<aside class="sidebar">
    <div class="sidebar-brand" style="font-weight: bold; font-family: 'Bookman Old Style'; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
